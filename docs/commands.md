...
| `lint` | `tidy` + `format`                                                                                                                                                          |
| `validate` | Запускает линтеры (`format` + `tidy`) + проверяет решение на наличие запрещенных паттернов (см. `forbidden_patterns` в конфиге задачи и в корневом конфиге `.clippy.json`) |
